...
const DEFAULT_RPC_URL = (typeof process !== "undefined" && (process.env?.VITE_RPC_URL || process.env?.RPC_URL)) || "http://127.0.0.1:8545";
...
export function createSupplyChainApi(options: {
  ...
  if (!registryArtifact?.address) {
    throw new Error("SupplyChainRegistry deployment artifact not found. Run the deploy script first.");
  }

  const registry = new Contract(registryArtifact.address, registryArtifact.abi, signer ?? provider);
  ...
  async function register(payload: Registration) {
    ...
    await tx.wait();
    const id = account ? `REG-${account}` : `REG-${await signer.getAddress()}`;
  ...
  async function createDeliveryRequest(payload: Omit<DeliveryRequest, "id">) {
    ...
    await tx.wait();
    return { ok: true, id: encodeRequestId(Number(previewId)) } as const;
  }
  ...
  async function findMatches(requestId: string): Promise<MatchCandidate[]> {
    ...
    return candidates.map((candidate) => ({
      ...
      capacityOK: candidate.capacityOk,
  ...
  async function draftContract(requestId: string, partyA: string, partyB: string, providerAddress?: string): Promise<SmartContractDraft> {
    ...
    const provider = providerAddress ?? (await signer.getAddress());
    ...
    return {
      id: encodeAgreementId(Number(agreementId)),
      requestId,
...
export const registryAddress: string | undefined = registryArtifact?.address;
